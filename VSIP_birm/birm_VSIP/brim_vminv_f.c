...
int birm_vminv_f(const float *x1, const float *x2, const int nx, float *y)
{
    if (!x1 || !x2 || !y)
    ...
    vsip_block_f *blk_y = vsip_blockbind_f((float *)y, nx, VSIP_MEM_NONE);
    ...
    // 逐元素求最小值: y[i] = min(x1[i], x2[i])
    vsip_vmin_f(vw_x1, vw_x2, vw_y);
